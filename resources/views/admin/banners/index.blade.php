<x-admin-layout header="Featured Banner Management">
    <div class="space-y-6">
        <!-- Header with Create Button -->
        <div class="flex justify-between items-center">
            <h2 class="text-2xl font-semibold text-gray-900">Featured Banners</h2>
            ...
            <form method="GET" action="{{ route('admin.banners.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <select name="link_type" 
                        ...
                        <option value="product" {{ ($filters['link_type'] ?? '') === 'product' ? 'selected' : '' }}>Product</option>
                        <option value="vendor" {{ ($filters['link_type'] ?? '') === 'vendor' ? 'selected' : '' }}>Vendor/Shop</option>
                        <option value="category" {{ ($filters['link_type'] ?? '') === 'category' ? 'selected' : '' }}>Category</option>
                        <option value="url" {{ ($filters['link_type'] ?? '') === 'url' ? 'selected' : '' }}>External URL</option>
                    </select>
                </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($banners as $banner)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-gray-900">{{ $banner->title ?? 'No Title' }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    @if($banner->link_type)
                                        <div class="text-sm text-gray-900 capitalize">{{ $banner->link_type }}</div>
                                        @if($banner->link_type === 'url' && $banner->external_url)
                                            <div class="text-xs text-gray-500 truncate max-w-xs">{{ $banner->external_url }}</div>
                                        @elseif($banner->link_id)
                                            <div class="text-xs text-gray-500">ID: {{ $banner->link_id }}</div>
                                        @endif
                                    @else
